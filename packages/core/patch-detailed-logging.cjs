...
content = content.replace(/this\.messages\.push\(/g, `this.client.getLogger().error('[DEBUG] Adding message:', JSON.stringify({ role: arguments[0]?.role || 'unknown', contentType: typeof arguments[0]?.content, stack: new Error().stack.split('\\n')[2] }, null, 2));\n      this.messages.push(`);
